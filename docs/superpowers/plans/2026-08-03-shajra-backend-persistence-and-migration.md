...
`GraphWriteSet` contains complete person, family-unit, and link versions generated
by pure commands, not database patches.

- [ ] **Step 4: Implement in-memory append-only semantics**

Store versions by `(logical_id, revision)` and commits by revision. `load_committed`
finds the highest commit, then selects the highest version at or below that
revision for each logical ID. It ignores every version whose revision lacks a
commit. `append_commit` rejects non-sequential revisions and stale fencing tokens.
...
`FencingToken`, `IsArchived`, and semantic fields. Never update an existing
...
compensation as a new operation.
...
`SourceRecordId`, `Airtable`, or record IDs beginning with `rec`.

- [ ] **Step 4: Implement API schemas and routers**

Map domain projection types to camel-case Pydantic response models. Keep current
v1 read routes as adapters until frontend cutover. Add admin routes:
